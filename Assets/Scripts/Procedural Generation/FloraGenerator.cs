    ...
    public class FloraGenerator : MonoBehaviour
    {
        ...
        public void GenerateTrees()
        {
            ...
            for (int y = 0; y < length - 1; y++)
            {
                ...
                {
                    if(terrainMap[x, y] == TerrainName.Grass)
                    {
        ...
        public void GeneratePlants()
        {
            ...
            for (int y = 0; y < length - 1; y++)
            {
                for (int x = 0; x < width - 1; x++)
                {
                    if (terrainMap[x, y] == TerrainName.Grass || terrainMap[x, y] == TerrainName.ShallowGrass)
                    {
                        float chance = pseudoRNG.Next(0, 100);
        ...
        public void Init(int seed, int width, int length, TerrainName[,] terrainMap)
        {
            this.seed = seed;
            this.width = width;
            this.length = length;
